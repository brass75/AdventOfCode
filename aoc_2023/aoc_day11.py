...
def expand(input_: list[list[str]]) -> tuple[list[int], list[int]]:
    return (
        [i for i, row in enumerate(input_) if '#' not in row],
...
def get_galactic_coordinates(map_: list[list[str]]) -> list[tuple[int, int]]:
    return [(i, j) for i, row in enumerate(map_) for j, c in enumerate(row) if c == GALAXY]
...
def solve(input_: str, expansion: int = 2) -> int:
    ...
    pairs = {tuple(sorted((start, dest))) for start in coordinates for dest in coordinates if start != dest}
