...
def min_distance(
    ...
    mult = sum(n in row_range for n in rows) * (expansion - 1) + sum(
        n in col_range for n in cols
    ) * (expansion - 1)
...
def expand(input_: list[list[str]]) -> tuple[list[int], list[int]]:
    return (
        ...
        [
            ...
            if not any(row[i] == GALAXY for row in input_)
        ],
    )
...
def get_galactic_coordinates(map_: list[list[str]]) -> list[tuple[int, int]]:
    return [
        (i, j) for i, row in enumerate(map_) for j, c in enumerate(row) if c == GALAXY
    ]
