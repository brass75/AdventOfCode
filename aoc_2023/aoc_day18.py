...
def solve(input_: str, use_instruction: bool = False) -> int:
    parsed = [line.split() for line in input_.splitlines()]
    mapped = []
    i, j, perimeter = 0, 0, 0
    for direction, length, instructions in parsed:
        if use_instruction:
            # The last character is `)` so we need the next to last to get the direction
            direction = instructions[-2]
            # The first 2 characters are `(#` so we need to skip them to get the length
            length = int(instructions[2:-2], 16)
        else:
            length = int(length)
        perimeter += length
        i, j = DIR_MAP[direction](i, j, length)
        mapped.append((j, i))
    # The polygon area doesn't include the perimeter properly so we need to add it here.
...
if __name__ == "__main__":
    ...
    part2_args = [True]
    expected_2 = [(952408144115, [TEST_INPUT, *part2_args])]
    func_2 = solve
